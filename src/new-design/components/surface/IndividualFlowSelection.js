...
export default function IndividualFlowSelection({ setSurfaceAnalyse }) {
    ...
    return (
        <TableCell sx={{ padding: "0px", px: "5px", fontSize: '0.857rem', lineHeight: "1.1rem", width: "100px" }}>
            <FormControl fullWidth>
                <Select
                    ...
                    sx={{ height: 20, fontSize: '0.714rem' }}
                >
                    {options.map((option) => (
                        <MenuItem key={'menu-' + option.value} value={option.value} sx={{ fontSize: '0.714rem' }}>
                            {option.name}
                        </MenuItem>
                    ))}
                </Select>

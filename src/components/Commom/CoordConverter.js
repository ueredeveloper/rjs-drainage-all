/**
 * Componente modal para conversão de coordenadas geográficas.
...
// Coordenadas UTM 23S do Marco Zero de Brasília
const BRASILIA_UTM = { zone: "23", hemisphere: "S", easting: "190756", northing: "8253264" };

// Coordenadas GMS de Brasília: 15°46'47"S  47°55'45"W
const BRASILIA_GMS = {
...
/**
 * Converte coordenadas UTM (SIRGAS 2000 / WGS 84) para graus decimais.
 * Implementa a série de Helmert para a projeção Transverse Mercator inversa.
 *
 * @param {number} zone - Número do fuso UTM (1–60).
 * @param {'N'|'S'} hemisphere - Hemisfério Norte ou Sul.
 * @param {number} easting - Coordenada E em metros.
 * @param {number} northing - Coordenada N em metros.
...
function utmToLatLng(zone, hemisphere, easting, northing) {
  ...
  // usa turf.point para validar que o resultado é um ponto geográfico válido
...
/**
 * Converte graus, minutos e segundos (GMS) para grau decimal.
 *
...
/**
 * Modal de conversão de coordenadas (UTM → Decimal e GMS → Decimal).
 * Pré-preenchido com as coordenadas do Marco Zero de Brasília (Fuso 23S).
 *
 * @component
 * @param {Object} props
 * @param {boolean} props.open - Controla a visibilidade do modal.
 * @param {Function} props.onClose - Callback para fechar o modal.
 * @param {Function} props.onConvert - Callback chamado com { lat, lng } ao aplicar a conversão.
 * @returns {JSX.Element}
 */
function CoordConverter({ open, onClose, onConvert }) {
